        ...
        {if $GroupCaption != 'Default'}
            <li class="dropdown">
                ...
                <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
                    {$Captions->GetMessageString($GroupCaption)}
        ...
        {foreach item=PageListPage from=$List.Pages}
            {if $PageListPage.GroupName == $GroupCaption}
                ...
                <li{if $PageListPage.ClassAttribute} class="{$PageListPage.ClassAttribute}"{/if}>
                    <a href="{$PageListPage.Href|escapeurl}" title="{$Captions->GetMessageString($PageListPage.Hint)}"{if $PageListPage.Target} target="{$PageListPage.Target}"{/if}>
                        {$Captions->GetMessageString($PageListPage.Caption)}
                    </a>
                </li>
            {/if}
        {/foreach}
