                ...
                <div class="dropdown-menu dropdown-menu-right" role="menu">
                    <form class="form-horizontal quick-filter">
                            ...
                            <div class="col-input" style="width:100%;max-width:100%" data-column="quick_filter_fields">
                                <select id="quick-filter-fields" class="form-control" name="quick_filter_fields[]" multiple data-max-selection-size="0" data-placeholder="{$Captions->GetMessageString('All')}" data-editor="multivalue_select">
                                    {foreach item=column from=$filter->getColumns()}
                                        <option value="{$column->getFieldName()}" {if $column->getFieldName()|in_array:$filter->getSelectedFieldNames()} selected{/if}>{$Captions->GetMessageString($column->getCaption())}</option>
                                    {/foreach}
                                </select>
                            </div>
                        </div>
                        ...
                        <span class="pull-right">
                            <button type="button" class="js-cancel btn btn-cancel">{$Captions->GetMessageString('Cancel')}</button>
                            <button type="button" class="js-submit btn btn-primary">{$Captions->GetMessageString('Apply')}</button>
                        </span>
                    </form>
                </div>
