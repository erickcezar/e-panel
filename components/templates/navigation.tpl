        ...
        {foreach from=$navigation item=item name=navigation}
            <li class="dropdown">
                {if $item.url and not $smarty.foreach.navigation.last}
                    <a href="{$item.url}">{$Captions->GetMessageString($item.title)}</a>
                {else}
                    {$Captions->GetMessageString($item.title)}
                {/if}
                ...
                {if $item.siblings and count($item.siblings) > 0}
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                        {foreach from=$item.siblings item=sibling}
                            <li><a href="{$sibling.url}">{$Captions->GetMessageString($sibling.title)}</a></li>
                        {/foreach}
                    </ul>
                {/if}
            </li>
