				...
				<div style="margin: 0px 20px;">
					...
					<div class="tab-content" style="margin-top: 20px; min-height: 350px">
						<div role="tabpanel" class="tab-pane active" id="page01-{$Grid.FormId}">
							...
							{include file="custom_templates/custom_form_view_column.tpl" Col=$Columns.description Help=description}
							{include file="custom_templates/custom_form_view_column.tpl" Col=$Columns.expire Help=expire}

							<div class="form-title-field">{$Captions->GetMessageString('Active')}</div>
							{include file="custom_templates/custom_form_view_column.tpl" Col=$Columns.active Help=active}
						</div>
					</div>	
				</div>
			</div>
