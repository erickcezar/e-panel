        ...
        foreach($columns as $column) {
            if ($column->allowSorting()) {
                $sortableColumn = array(
                    ...
                    'caption' => $parentPage->GetLocalizerCaptions()->GetMessageString($column->getCaption()),
                );
                $sortableColumns[$column->getFieldName()] = $sortableColumn;
                $sortableColumnsForJSON[$column->getName()] = array_merge($sortableColumn, array(
                    'caption' => StringUtils::ConvertTextToEncoding($parentPage->GetLocalizerCaptions()->GetMessageString($column->getCaption()), $parentPage->getContentEncoding(), 'UTF-8')
                ));
            }
        }
