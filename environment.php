 ...
 */
    ...
    class environmentPage extends Page
    {
        ...
        {
            $this->SetTitle('TitleEnvironment');
            $this->SetMenuLabel('LabelEnvironment');
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            ...
            $column->setAlign('left');
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $column->SetDescription('header_hint_uuid');
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $editColumn->GetCaption()));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $validator = new CustomRegExpValidator('^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$', StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RegExpValidationMessage'), $editColumn->GetCaption()));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        }
    ...
    try
    {
        $Page = new environmentPage("environment", "environment.php", GetCurrentUserPermissionSetForDataSource("environment"), 'UTF-8');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("environment"));
